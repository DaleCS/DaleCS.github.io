...
const Contact = forwardRef(({ isVisible, breakpoint }, ref) => {
  const [surfaceAnims, setSurfaceAnims] = useState(new Array(NUM_OF_CONTACTS).fill(false));
  ...
  useEffect(() => {
    if (!previouslyLoaded.current && isVisible) {
      ...
      const animsInterval = setInterval(() => {
        ...
        console.log(surfaceAnimsRef.current);
  ...
  return (
    ...
    >
      ...
      <div className={`contact-outer-container--${breakpoint}`}>
        <div title="My Resume" onClick={handleOnClickResume} className={`resume-surface ${surfaceAnims[0] ? "contact-surface-appear" : ""}`}>
          <img src={ResumeIcon} className={`icon--${breakpoint}`} alt="My Resume" />
          <span className={`surface-text--${breakpoint}`}>My Resume</span>
          <span className={`surface-text--${breakpoint}`}>(.pdf, ~137KB)</span>
        </div>
        <div title="mailTo: Dale.Seen@gmail.com" onClick={handleOnClickEmail} className={`other-contact-surface ${surfaceAnims[1] ? "contact-surface-appear" : ""}`}>
          <img src={EmailIcon} className={`icon--${breakpoint}`} alt="Email" />
          <span className={`surface-text--${breakpoint}`}>Email</span>
        </div>
        <div title="My LinkedIn" onClick={handleOnClickLinkedIn} className={`other-contact-surface ${surfaceAnims[2] ? "contact-surface-appear" : ""}`}>
          <img src={LinkedInIcon} className={`icon--${breakpoint}`} alt="LinkedIn" />
          <span className={`surface-text--${breakpoint}`}>LinkedIn</span>
        </div>
        <div title="My Github" onClick={handleOnClickGithub} className={`other-contact-surface ${surfaceAnims[3] ? "contact-surface-appear" : ""}`}>
          <img src={GithubIcon} className={`icon--${breakpoint}`} alt="Github" />
